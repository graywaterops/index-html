/* donor-universe.js — Three.js donor universe
   - Play/Pause truly pauses spin AND counting
   - Read density from HTML data-attributes on <canvas id="gl">
       data-seeds="80"   (roots)
       data-extras="2"   (greens per root, rough avg)
       data-reds="2"     (reds per green, rough avg)
   - Wires #btnPlay, #btnAll, #btnDebug on your page
*/
...
(() => {
  ...
  // ---- read knobs from HTML (no JS edits needed) ----
  const cfg = {
    seeds  : +canvas.dataset.seeds  || 250,  // roots
    extras : +canvas.dataset.extras || 3,    // ~greens / root
    reds   : +canvas.dataset.reds   || 4     // ~reds / green
  };

  // ---- load three + controls (robust) ----
  ...
  (async ()=>{
    ...
    function add(type,parent=null){ const n={id:nid++,type,parent,x:0,y:0,z:0,children:[]}; nodes.push(n); if(parent!=null){links.push({source:parent,target:n.id}); nodes[parent].children.push(n.id);} return n;}
    ...
    for(let i=0;i<cfg.seeds;i++) add('dark',null);
    const roots=nodes.filter(n=>n.type==='dark');
    const rootPts=fib(roots.length,RADIUS); roots.forEach((n,i)=>{ n.x=rootPts[i].x; n.y=rootPts[i].y; n.z=rootPts[i].z; });

    function poissonLike(avg){ // small integer around avg
      const base = Math.max(0, Math.round(avg + (Math.random()*2-1)));
      return base;
    }

    for(const r of roots){
      const l=add('light',r.id); l.x=r.x+rnd(); l.y=r.y+rnd(); l.z=r.z+rnd();
      const greens = Math.max(0, poissonLike(cfg.extras));
      for(let g=0; g<greens; g++){
        const gn=add('green',r.id); gn.x=r.x+rnd(); gn.y=r.y+rnd(); gn.z=r.z+rnd();
        const reds = Math.max(0, poissonLike(cfg.reds));
    ...
    const edges=new THREE.LineSegments(edgeGeo, new THREE.LineBasicMaterial({color:EDGE,transparent:true,opacity:0.4}));
    scene.add(edges);
    ...
    function clearHi(){
      ...
      edges.material.opacity = edges.visible ? 0.4 : 0.02;
    ...
    function highlight(startId){
      ...
      scene.add(glow);
      setStatus(`selected node ${startId} — downline highlighted`);
    }
    function onPick(ev){
    ...
    // ---- Buttons: make them control everything ----
    let playing = true;      // master play/pause (spin + counting)
    let showEdges = true;
    let yellow = false;
    ...
    function setYellow(on){
      ...
      else {  MATS.dark.color.setHex(COLOR.dark);
    ...
    // ---- Counters + progress (follow master "playing") ----
    ...
    // ---- Animate ----
    setStatus('ready — rotate, pan, zoom, click to highlight. Use the buttons to control.');
    function loop(){
      if(controls && 'autoRotate' in controls) controls.autoRotate = !!playing;
