/* donor-universe.js — WebGL triangle → auto 3D galaxy (round, color-coded stars) + working controls + 2D fallback */
...
(() => {
  ...
  // --- ensure canvas area has height (Squarespace guard) ---
  ...
  // --- sanity: required DOM ---
  (function verifyDOM(){const miss=[]; if(!statusEl) miss.push('#status'); if(!coinsEl) miss.push('#coins'); if(!raisedEl) miss.push('#raised'); if(!glCanvas) miss.push('#gl'); if(!cv2d) miss.push('#cv2d'); if(miss.length){ setStatus('Error: missing DOM ids: '+miss.join(', ')); }})();

  // --- get WebGL or fallback ---
  ...
  function runGalaxy(gl){
    ...
    setStatus('Galaxy running — rotate (drag), pan (Shift/Right/Ctrl-drag), wheel to zoom.');

    // ---- build donor model (roots + primary + extras + extra descendants) ----
    const P={ roots:250, radius:800, jitter:36, cap:15000, seed:'universe-v3' };
    ...
    // per-type color & size (smaller to avoid blobs)
    ...
    const SIZE= { dark:6.0, light:4.2, green:4.8, red:3.6 };

    // Interleaved vertices per star: center(3), corner(2), sizePx(1), color(3) => 9 floats
    ...
    // Shader: simple yaw/pitch/scale/pan/zoom; circular mask for stars; debug yellow toggle
    const vsStars=`
    ...
    if (btnAll)  btnAll.onclick  = ()=> { showEdges=!showEdges; btnAll.textContent = showEdges?'Hide Edges':'Show Edges'; autoFit(); };
    if (btnDbg)  btnDbg.onclick  = ()=> { yellow=!yellow; btnDbg.textContent = yellow?'Normal Dots':'Yellow Dots'; };

    // Mouse interactions
    let dragging=false,rotating=false,lastX=0,lastY=0;
    glCanvas.addEventListener('contextmenu', e=> e.preventDefault());
    glCanvas.addEventListener('mousedown',e=>{ dragging=true; rotating=!(e.ctrlKey||e.shiftKey||e.button===2); lastX=e.clientX; lastY=e.clientY; });
    ...
    function autoFit(){ panPxX=0; panPxY=0; yaw=0; pitch=0; zoom=1.0; }

    // draw loop
    (function draw(){
      resizeGL();
      if(autoSpin) yaw += 0.003; // slow spin so Play/Pause is visible
      gl.clearColor(0,0,0,1); gl.clear(gl.COLOR_BUFFER_BIT);
      ...
      if(showEdges){
        ...
        gl.uniform4f(locLines.uCol, 0.36,0.42,0.58, 0.28);
    ...
    // simple counters to show life (no server yet)
  ...
  // grab GL early (used by resizeGL)
  let gl=null; try { gl=glCanvas.getContext('webgl',{antialias:true,alpha:false}); } catch {}
})();
