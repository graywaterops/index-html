/* donor-universe.js — Three.js 3D donor universe
   ...
   - <canvas id="gl">
   - #btnPlay, #btnAll, #btnDebug
   - #coins, #raised, #status, #fill (optional)
*/
...
(() => {
  ...
  const fillEl   = $('fill');   // progress bar fill (optional)
    ...
    try{
      ...
      if(btnPlay){
        btnPlay.onclick = ()=>{
          ...
          if (controls && 'autoRotate' in controls) controls.autoRotate = autoSpin; // OrbitControls
        };
      }
      if(btnAll){
        btnAll.onclick = ()=>{
          ...
          edges.material.opacity = showEdges ? 0.40 : 0.02;  // clear difference
      ...
      function loop(){
        if(controls && 'autoRotate' in controls) controls.autoRotate = !!autoSpin;
        if(!('autoRotate' in controls) && autoSpin){
