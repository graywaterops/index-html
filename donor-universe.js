/* donor-universe.js — Three.js 3D donor universe
   Works with your page:
   - <canvas id="gl"> for rendering
   - #btnPlay (Play/Pause spin)
   - #btnAll (Show/Hide edges, clear highlight)
   - #btnDebug (Yellow Dots toggle)
   - #coins, #raised, #status (optional)
*/
...
(async ()=>{
  ...
  if(OrbitControls){
    ...
    controls.autoRotate = true;           // start spinning
  ...
  /* ---- Build donor graph (roots + primary + extras + extra-descendants) ---- */
  ...
  /* ---- Buttons: Play/Pause, Show All (edges), Yellow Dots ---- */
  let autoSpin = true;
  let showEdges = true;
  ...
  function setYellow(on){
    yellowMode = !!on;
    if(on){
      for(const k of Object.keys(MATS)) MATS[k].color.setHex(0xffff00);
    }else{
  ...
  if(btnPlay){
    btnPlay.onclick = ()=>{
      ...
      if(controls && 'autoRotate' in controls) controls.autoRotate = autoSpin; // OrbitControls path
    };
  }
  if(btnAll){
    btnAll.onclick = ()=>{
      ...
      edges.material.opacity = showEdges ? 0.40 : 0.02;  // make the change obvious
  ...
  /* ---- Counters + progress (front-end demo) ---- */
  ...
  function loop(){
    if(controls && 'autoRotate' in controls) controls.autoRotate = !!autoSpin; // keep OC in sync
    if(!('autoRotate' in controls) && autoSpin){ // fallback: small spin
...
})().catch(e=>{ console.error(e); const s=document.getElementById('status'); if(s) s.textContent='Status: Error: '+(e?.message||e); });
