(() => {
  ...
  const COLORS = {
    ...
    inactiveOutline: 0xffdd00,          // yellow leaf outline
  ...
  // small bias: grow more under extra/down branches
  ...
  function generateUniverse(total = 1000, seedRoots = 250) {
    ...
    // mark leaves (without changing their type)
  ...
  function makeNodeObject(n){
    ...
    // optional yellow wireframe outline for leaves
    const outlineGeo = new THREE.SphereGeometry(1.01, 12, 12);
    const outlineMat = new THREE.MeshBasicMaterial({ color: COLORS.inactiveOutline, wireframe: true, transparent: true, opacity: 0.95 });
  ...
  function draw({nodes, links}){
    Graph = ForceGraph3D()(container)
      ...
      // …then set data so objects are created with our renderer
  ...
  function tryFindAndFocus(raw){
    ...
    const waitForPos = () => (Number.isFinite(node.x) ? Promise.resolve() :
      new Promise(res => setTimeout(() => res(waitForPos()), 100)));
