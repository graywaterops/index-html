(() => {
  ...
  async function ensureLibs(){ if(!window.THREE) await loadScript('https://unpkg.com/three@0.155.0/build/three.min.js');
                               if(!window.ForceGraph3D) await loadScript('https://unpkg.com/3d-force-graph@1.71.6/dist/3d-force-graph.min.js'); }
  ensureLibs().then(start).catch(e=>{const st=document.getElementById('status'); if(st) st.textContent=`Failed to load libs: ${e.message}`; console.error(e);});
  ...
  function start(){
    ...
    let highlightNodes=new Set(), highlightLinkKeys=new Set();

    let nodeSize=22;          // bigger default
    let universeSpread=150;   // wider default
    let zoomDist=280;         // camera snap distance
    let heatByDonation=false; // off by default
    let isolateView=false;
    let showLinksAll=false;   // NEW: if false, show links only on selection
    ...
    // ---------- data ----------
    function randomDonation(){const r=Math.random(); if(r<.75) return Math.floor(50+Math.random()*50);
      if(r<.95) return Math.floor(100+Math.random()*400); return Math.floor(500+Math.random()*4500);}
    function pickBiasedParent(){const pool=nodes.filter(n=>n.type==='extra'||n.type==='down'); if(pool.length&&Math.random()<.35) return pool[Math.floor(Math.random()*pool.length)]; return nodes[Math.floor(Math.random()*nodes.length)];}
    function generateUniverse(total=3200, seedRoots=250){
      nodes=[]; links=[]; byId=new Map(); let id=0;
      for(let i=0;i<seedRoots;i++){const n={id:id++, type:'root', donation:randomDonation(), children:[], parent:null, inactive:false}; nodes.push(n); byId.set(n.id,n);}
      for(let i=seedRoots;i<total;i++){const p=pickBiasedParent(), d=randomDonation(); let type='primary';
        if(p.children.length>0) type= p.type==='primary'?'extra':'down';
        const c={id:id++, type, donation:d, children:[], parent:p.id, inactive:false};
        nodes.push(c); byId.set(c.id,c); p.children.push(c.id); links.push({source:p.id,target:c.id});}
      nodes.forEach(n=>n.inactive = (n.children.length===0));
      minDonation=Math.min(...nodes.map(n=>n.donation)); maxDonation=Math.max(...nodes.map(n=>n.donation));
      return {nodes,links};
    }

    // ---------- helpers ----------
    function getBloodlineTotal(rootId){let t=0;const seen=new Set();(function dfs(id){if(seen.has(id))return;seen.add(id);const n=byId.get(id);if(!n)return;t+=n.donation||0;n.children.forEach(dfs);})(rootId);return t;}
    function getSubtreeStats(rootId){let c=0,t=0,d=0;(function dfs(id,dep){c++;d=Math.max(d,dep);const n=byId.get(id);if(!n)return;t+=n.donation||0;n.children.forEach(x=>dfs(x,dep+1));})(rootId,0);return {count:c,total:t,depth:d};}
    function collectSubtree(rootId){const rows=[];(function dfs(id){const n=byId.get(id);if(!n)return;rows.push({id:n.id,type:n.type,donation:n.donation,parent:n.parent,inactive:n.inactive});n.children.forEach(dfs);})(rootId);return rows;}
    ...
    const heatHex=n=>{const t=(n.donation-minDonation)/Math.max(1,(maxDonation-minDonation)); const r=Math.round(255*t), g=Math.round(255*(1-0.3*t)); return rgbHex(r,g,60);};
    ...
    const baseRadius=n=> heatByDonation? Math.max(6, nodeSize*(n.donation/100)) : nodeSize;
    function radiusFor(n){ const b=baseRadius(n); if(!selectedNode) return b; if(!highlightNodes.has(n.id)) return b*0.8; return n.id===selectedNode.id? b*2.6 : b*1.8; }

    function linkKey(l){const s=typeof l.source==='object'?l.source.id:l.source; const t=typeof l.target==='object'?l.target.id:l.target; return `${s}->${t}`;}
    function linkVisible(l){
      const sId=typeof l.source==='object'?l.source.id:l.source, tId=typeof l.target==='object'?l.target.id:l.target;
      const s=byId.get(sId), t=byId.get(tId); if(!s||!t) return false;
      if(!nodeShouldDisplay(s)||!nodeShouldDisplay(t)) return false;
      if(selectedNode) return showLinksAll || highlightLinkKeys.has(`${sId}->${tId}`);
      return showLinksAll;
    }

    // ---------- selection / camera ----------
    function clearHighlights(){highlightNodes.clear(); highlightLinkKeys.clear(); selectedNode=null; updateAllNodeObjects(); Graph.refresh(); updateExportState(); syncQuery();
    ...
    function highlightPath(node){
      ...
      const down=id=>{highlightNodes.add(id);links.forEach(l=>{if(l.source===id){highlightLinkKeys.add(`${l.source}->${l.target}`);down(l.target);}});};
      const up=id=>{links.forEach(l=>{if(l.target===id){highlightLinkKeys.add(`${l.source}->${l.target}`);highlightNodes.add(l.source);up(l.source);}});};
    ...
    function focusCamera(node){ if(!node) return; const d=zoomDist; Graph.cameraPosition({x:node.x+d,y:node.y+d*0.8,z:node.z+d},{x:node.x,y:node.y,z:node.z},650); }
    ...
    function draw({nodes,links}){
      Graph = ForceGraph3D()(container)
        ...
        .nodeThreeObject(n=>{
          ...
          n.__fill=m; n.__obj=m; return m;
        })
        .nodeLabel(n=>{const tot=getBloodlineTotal(n.id); return `<div><b>${n.type.toUpperCase()}</b><br/>Coin #: ${n.id}<br/>Donation: ${money(n.donation)}<br/><b>Bloodline Total:</b> ${money(tot)}</div>`;})

        // visibility + style for links
        .linkVisibility(linkVisible)
        .linkColor(l=> highlightLinkKeys.has(linkKey(l)) ? COLORS.forward : 'rgba(180,180,180,0.22)')
        .linkWidth(l=> highlightLinkKeys.has(linkKey(l)) ? 3.6 : 0.35)
        .linkOpacity(l=> highlightLinkKeys.has(linkKey(l)) ? 1 : (showLinksAll ? 0.1 : 0.0))
      ...
      try{ Graph.d3Force('charge').strength(-universeSpread); Graph.d3Force('link').distance(universeSpread).strength(0.45);}catch(_){}
      const ctrl=Graph.controls&&Graph.controls(); if(ctrl){ ctrl.zoomSpeed=3.2; ctrl.minDistance=40; ctrl.maxDistance=5000; ctrl.enableDamping=true; ctrl.dampingFactor=0.12; }
    ...
    const sliderNode=document.createElement('input'); sliderNode.type='range'; sliderNode.min=8; sliderNode.max=80; sliderNode.value=nodeSize;
    ...
    const sliderSpread=document.createElement('input'); sliderSpread.type='range'; sliderSpread.min=80; sliderSpread.max=320; sliderSpread.value=universeSpread;
    sliderSpread.oninput=e=>{universeSpread=+e.target.value; try{Graph.d3Force('charge').strength(-universeSpread); Graph.d3Force('link').distance(universeSpread).strength(0.45);}catch(_){} Graph.refresh(); syncQuery();};
    ...
    const sliderZoom=document.createElement('input'); sliderZoom.type='range'; sliderZoom.min=120; sliderZoom.max=900; sliderZoom.value=zoomDist;
    ...
    function updateExportState(){ if(selectedNode){exportBtn.style.opacity='1';exportBtn.style.cursor='pointer';exportBtn.disabled=false;} else {exportBtn.style.opacity='.6';exportBtn.style.cursor='not-allowed';exportBtn.disabled=true;} }
    exportBtn.addEventListener('click',()=>{if(!selectedNode) return; const rows=collectSubtree(selectedNode.id);
    ...
    TYPES.forEach(([key,label])=>{const w=document.createElement('label'); w.style.display='flex'; w.style.alignItems='center'; w.style.gap='.35rem';
      const c=document.createElement('input'); c.type='checkbox'; c.checked=true;
      c.addEventListener('change',()=>{ if(c.checked) visibleTypes.add(key); else visibleTypes.delete(key); updateAllNodeObjects(); Graph.refresh(); syncQuery();});
      w.appendChild(c); w.appendChild(document.createTextNode(label)); filters.appendChild(w);});
    document.body.appendChild(filters);

    // find + URL
    function tryFindAndFocus(raw){const id=Number(String(raw||'').replace(/\D/g,'')); if(!Number.isFinite(id)) return pulse(findInput,'#ff6b6b');
    ...
    function applyQuery(){const p=new URLSearchParams(location.search);
      ...
      const qTypes=p.get('types'); if(qTypes){visibleTypes.clear(); qTypes.split(',').forEach(t=>{if(t)visibleTypes.add(t);});
