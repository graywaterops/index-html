(() => {
  ...
  function start() {
    ...
    function baseRadius(n){
      const r = heatByDonation ? Math.max(4, nodeSize*(n.donation/100)) : nodeSize;
      return r;
    }
    function radiusFor(n){
      // Enlarge the selected path so it pops
      const base = baseRadius(n);
      if (!selectedNode) return base;
      if (!highlightNodes.has(n.id)) return base * 0.7;         // slightly smaller background
      return n.id === selectedNode.id ? base * 2.4 : base * 1.6; // bigger along the path
    }
    ...
    function focusCamera(node){
      if(!node) return;
      const dist=zoomDist;
      Graph.cameraPosition({x:node.x+dist,y:node.y+dist*0.8,z:node.z+dist},{x:node.x,y:node.y,z:node.z},650);
    }
    ...
    function draw({nodes,links}){
      Graph = ForceGraph3D()(container)
        ...
        // Link styling — strong on path
        .linkColor(l => {
          const key = linkKey(l);
          if (selectedNode) return highlightLinkKeys.has(key) ? COLORS.forward : COLORS.faded;
          return "rgba(180,180,180,0.25)";
        })
        .linkWidth(l => highlightLinkKeys.has(linkKey(l)) ? 3.6 : 0.5)
        .linkOpacity(l => highlightLinkKeys.has(linkKey(l)) ? 1 : (selectedNode ? 0.08 : 0.22))
        // Animated particles moving along highlighted links
      ...
      if (ctrl) {
        ...
        ctrl.zoomSpeed   = 3.2;   // faster scroll zoom
    ...
    function updateAllNodeObjects(){
      nodes.forEach(n => {
        ...
        // scale for visibility and selection
        const r = Math.max(0.001, radiusFor(n));
        fill.scale.set(r, r, r);

        // color strictly by type or heat
        fill.material.color.setHex(fillHex(n));

        // background dimming by opacity
        if (selectedNode) {
          fill.material.opacity = highlightNodes.has(n.id) ? 1.0 : 0.10;
        } else {
    ...
    sliderNode.type="range"; sliderNode.min=6; sliderNode.max=64; sliderNode.value=nodeSize;
    ...
    sliderSpread.type="range"; sliderSpread.min=60; sliderSpread.max=300; sliderSpread.value=universeSpread;
    ...
    sliderZoom.type="range"; sliderZoom.min=100; sliderZoom.max=800; sliderZoom.value=zoomDist;
